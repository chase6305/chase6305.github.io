"""Single local image with the native InternVL 3.5 HF checkpoint.

Not the custom model.chat() API, not Flash, and not a benchmark.
...
def main(argv=None):
    ...
    parser.add_argument("--question", default="Describe the image. If text is unreadable, say so.")
    ...
    if not 1 <= args.max_input_tokens <= 28672:
        parser.error("Input cap must be in [1,28672], reserving a 4096-token output budget")
    ...
    if transformers.__version__ != "4.55.0":
        parser.error("This pinned example requires transformers==4.55.0")
    ...
    if prompt_length > args.max_input_tokens:
        parser.error(f"Input has {prompt_length} tokens; reduce visual or text input")
