...
| 自己运行验证 | [梯度实验](#run)、[真实参数更新](#parameter-update)、[迁移清单](#implementation) |
...
下载 [gspo_lab.py](gspo_lab.py)、[test_gspo_lab.py](test_gspo_lab.py)、[gspo_update.py](gspo_update.py) 和 [requirements.txt](requirements.txt)，放在同一目录。使用 Python 3.10+：
...
本文实际运行环境为 PyTorch 2.8.0+cu128，全部实验张量仍在 CPU 上；上面的安装命令选择 CPU wheel，不需要 GPU。八项测试检查正负优势的单侧裁剪、变长回答下 GSPO-token 的一阶梯度等价、old 与优势停止梯度、NaN padding 的屏蔽、零方差组与空回答，以及 current=old 时的梯度一致性；另外用有限差分独立核对梯度，并检查实际参数更新进入裁剪平台的行为。
...
- 运行八项测试，解释正负优势的裁剪方向，以及 GSPO-token 与 GSPO 一阶梯度等价的条件。
...
- v1 提供单一 cold-start 模型设置下的主要曲线证据；教学脚本不涉及 MoE、在线采样或跨引擎吞吐。
