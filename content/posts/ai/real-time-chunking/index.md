...
在原始 RTC 的单路、连续、无重叠推理请求设置下，若每次延迟都固定为 $d$，下一次请求不能比当前推理结束更早开始，还要求 $s\ge d$。两式合起来：
...
### 2.4 延迟波动时，要比较相邻两次请求 {#variable-latency}

将上面的理想化时序推广到不同请求：第 $j$ 次请求的观测起点为 $t_j$，从该起点到结果可用经过 $d_j$ 步；相邻起点间隔为 $s_j=t_j-t_{j-1}$。沿用新鲜观测、同一动作网格、旧块覆盖 $H$ 步且不计额外安装开销的假设，串行请求与旧块覆盖分别要求：

$$
s_j\ge d_{j-1},\qquad s_j+d_j\le H.
$$

前一个不等式使用**上一请求**的延迟，后一个使用**当前请求**的延迟。存在可行间隔的条件因此是 $d_{j-1}+d_j\le H$；只有固定延迟时才简化为 $2d\le H$。

例如 $H=8$，前后延迟为 2、5，取起点间隔 2，旧块仍可覆盖等待过程，虽然当前延迟 5 已超过 $H/2$。如果连续两次延迟都是 5，则既要间隔至少 5，又要间隔至多 3，无法满足；若前后延迟为 5、2，取间隔 5 又可以覆盖。

这是本文根据时间轴作出的推导，由 [rtc_lab.py](rtc_lab.py) 验证；没有改变固定 Kinetix 评测中同一个 `d` 同时用于各轮的事实。真实部署还要加入观测陈旧、控制器承诺和调度开销，不能仅凭某次慢请求成功被覆盖，就认为同样的高延迟能够长期维持。预测延迟用于提前规划，实际延迟用于事后核对，两者也不能混成一个观测值。
...
if result.session_id != active_session_id:
    discard(result)                  # 上一轮任务或重置前的结果
elif result.request_id <= installed_request_id:
...
### 6.4 episode 重置后，旧请求编号再大也不能安装 {#session-boundary}

请求编号只能比较同一控制会话中的先后。假设任务 A 发出请求 99，随后环境重置、控制步归零，任务 B 已安装请求 2。A 的结果此时返回：仅检查 `99 > 2` 会通过；若旧结果的观测步也碰巧落在新一轮的时间范围内，时间检查同样无法识别它属于旧任务。

因此上面的接口先检查独立的 `session_id`，再检查会话内部的请求编号。会话标识可以由客户端绑定到请求上下文，不必作为动作网络的输入。episode 重置、任务语义变化，或者模型／动作适配配置被切换时，应明确是否开启新会话。编号必须不与仍在途的旧会话复用；服务重启后只把计数器恢复成 0，也不能自动满足这个要求。

| 事件 | 需要一致更新的运行时状态 |
| --- | --- |
| 开始新会话 | 会话标识、请求顺序、观测／动作时间原点、可变队列 |
| 旧结果迟到或重复返回 | 拒绝结果，不改变已安装编号与当前队列 |
| 开始下一次生成 | 从新会话的观测和实际承诺构造条件，不复用旧任务前缀 |
| 控制器还有不可撤回命令 | 按执行系统的停止／完成协议处理；清空软件列表不会撤回驱动已接收的命令 |

检查会话、计算可替换边界、更新队列与已安装编号要在同一次受保护的更新中完成。取消 HTTP 请求或 Future 可以减少无用工作，但不能代替返回结果的会话检查：远端计算可能已经开始，取消也可能与结果返回竞争。

这部分是本文补充的部署设计，未宣称 Kinetix 的延迟模拟实现了分布式任务取消。随文 [rtc_lab.py](rtc_lab.py) 覆盖旧会话高编号、重复结果、乱序结果及有效新结果的确定性反例；它只检查安装决策，不测试线程竞争。对于循环策略，新会话还要处理隐藏状态与历史输入，见 [Light-Loco-Parkour 的状态生命周期]({{< relref "/posts/ai/light-loco-parkour" >}}#selective-reset)。
...
#### 队列不断供，也可能一直使用陈旧观测 {#observation-age}

覆盖余量衡量“还有没有动作可执行”，观测年龄衡量“这些动作依据多早的信息”。设观测采样、请求发出、结果可安装的时刻分别为 $t_{capture},t_{send},t_{install}$，则安装时观测年龄为：

$$
A_{obs}=t_{install}-t_{capture}
=(t_{send}-t_{capture})+(t_{install}-t_{send}).
$$

即使第二项相同，相机缓冲造成的第一项仍会改变计划的新鲜度。以下教学例子采用本文的同网格约定：每步 20 ms、新块覆盖从观测时刻开始的 16 步，即半开时间区间长 320 ms；请求发出时旧队列都还覆盖 200 ms。

| 情况 | 发请求前观测已陈旧 | 请求至可安装 | 安装时观测年龄 | 新块未过期时间范围 |
| --- | ---: | ---: | ---: | ---: |
| 新采样的观测 | 0 ms | 80 ms | 80 ms | 240 ms，12 步 |
| 相机缓冲积压 | 200 ms | 80 ms | 280 ms | 40 ms，2 步 |

两种情况的旧队列都还剩 120 ms，均未断供；新结果可用的未来范围却相差很大。若请求前观测已经陈旧 240 ms，结果到达时整块刚好过期。增大“推理等待”的前缀估计，不能单独消除采样之后、请求发出之前的缓存积压。

因此诊断时应同时记录旧队列覆盖、新块到期边界和观测年龄，并说明时间戳来自采样、接收还是预处理完成。这个例子由随文实验验证时间算术，没有测量真实相机或 GPU，也没有声称某个观测年龄阈值适合所有机器人。
...
- 解释 H、d、s 的时间约定，区分观测年龄与队列覆盖，并按会话和不可撤回边界安装新块后缀。
