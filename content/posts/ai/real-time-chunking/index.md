...
![机器人执行橙色已承诺动作时，策略在后台根据相机观测规划青色后续动作，经过动作队列交给控制器](assets/rtc-overview.png "图 1：橙色前缀来自已有计划，不能由新结果追溯修改；曲线和方块数量不表示实测轨迹、具体块长或调度周期。")
...
![两个动作块在绝对时间上对齐，新块前四步为已承诺前缀，中间七步仍与旧块重叠，最后五步为新增未来](assets/chunk-timeline.svg "图 2：时间轴取 H=16、s=5、d=4。橙色前缀由旧块执行；新结果返回后，从新块索引 4 开始使用，而不是从索引 0 重播。")
...
![同一观测进入世界动作模型，输出对齐的未来视频与动作预测；后续真实观测仍需要传感器采集](assets/wam-video-action.png "图 5：画面与动作图标均为示意，不是论文的模型输出、实验照片或精确网络结构。")
