#!/usr/bin/env python3
"""Regenerate the article's original SVG diagrams; standard library only."""
...
def svg_start(title,desc,height):
    return f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1100 {height}" role="img" aria-labelledby="title desc"><title id="title">{title}</title><desc id="desc">{desc}</desc><defs><marker id="arrow" markerWidth="9" markerHeight="9" refX="7" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 Z" fill="#526782"/></marker></defs><rect width="1100" height="{height}" rx="22" fill="#f8faff"/><g font-family="{font}" fill="#18334f">'
...
def box(x,y,w,h,heading,lines,fill='#e5effb'):
    s=f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="16" fill="{fill}" stroke="#b4c7dd"/>'
    s+=text(x+20,y+36,heading,24,True)
    for i,line in enumerate(lines):s+=text(x+20,y+73+i*31,line,21)
    return s
def arrow(x1,y1,x2,y2):
    return f'<path d="M{x1},{y1} L{x2},{y2}" fill="none" stroke="#526782" stroke-width="3" marker-end="url(#arrow)"/>'
...
for y,a,al,b,bl,c,cl in rows:
    s+=box(35,y,300,140,a,al)+arrow(341,y+70,385,y+70)+box(395,y,310,140,b,bl,'#ede8fb')+arrow(711,y+70,755,y+70)+box(765,y,300,140,c,cl,'#fff0d9')
...
s+=box(585,90,480,135,'2  可交互环境',['碰撞、质量、摩擦、关节与接触','区分视觉背景和可交互对象'],'#ede8fb')
s+=arrow(522,155,575,155)+arrow(825,232,825,282)
s+=box(585,295,480,140,'3  执行接口与任务定义',['控制频率、执行器、传感器与延迟','初始状态、成功条件、失败与重置'],'#ede8fb')
s+=arrow(575,365,525,365)
s+=box(35,295,480,140,'4  在环境中实际执行',['人工示范 / 规划器 / 策略 rollout','记录命令、状态转移与观察'],'#e5effb')
s+=arrow(275,442,275,492)
s+=box(35,505,480,140,'5  数据检查与训练',['同步、字段有效性、派生关系','分组划分后训练与消融'],'#fff0d9')
s+=arrow(522,575,575,575)
s+=box(585,505,480,140,'6  独立真机评测',['检查预测、规划与任务执行表现','将失败反馈到环境与数据设计'],'#fff0d9')
...
s+=f'<circle cx="{x(1000)}" cy="195" r="8" fill="#227d78"/><circle cx="{x(1020)}" cy="195" r="8" fill="#227d78"/>'
...
s+=f'<circle cx="{x(1025)}" cy="330" r="8" fill="#ba7c26"/><circle cx="{x(1080)}" cy="330" r="8" fill="#ba7c26"/>'
s+=text(x(1025)-25,301,'采集',20)+text(x(1080)-20,376,'到达',20)
s+=f'<path d="M{x(1040)},118 V570" stroke="#18334f" stroke-width="3" stroke-dasharray="9 7"/>'
...
for k,ms in enumerate((1040,1060,1080)):
 s+=f'<rect x="{x(ms)}" y="447" width="126" height="82" rx="8" fill="#ede8fb" stroke="#b8abd6"/>'
