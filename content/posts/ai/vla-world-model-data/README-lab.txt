...
博客仓库维护者可在仓库根目录运行：
  python scripts/package_robot_data_lab.py --check --test
该命令只读检查归档与哈希，再在临时解压目录运行 30 项实验及重建 SVG。
正文附属文件更新后，去掉 --check 可重建 ZIP 与 SHA256SUMS。
此维护脚本位于博客仓库 scripts/，不包含在下载实验包内；读者直接运行上面的核心实验即可。
