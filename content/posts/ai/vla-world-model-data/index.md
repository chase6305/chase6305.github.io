...
### 4.1 ALOHA、ACT 与 GELLO：采集装置和策略分开看
...
![左侧人手持带相机的平行夹爪抓杯子，右侧机器人使用相似工具，中央图标分别表示坐标、时间与可达性检查](assets/umi-interface-transfer.png "图 4：从手持工具示范到机器人执行。左右两侧保持相近的夹爪接触方式与局部观察，但工具的运动分别由人和机器人提供。中央三项检查对应坐标表示、时序匹配和机器人可达性；相似的工具外形不能自动完成迁移。硬件为概念示意。")
...
![三个独立画面分别展示普通人类视频、带手部姿态追踪的第一视角记录，以及具有动作轨迹的机器人操作](assets/human-video-supervision.png "图 5：相同的抓取行为可以对应三种监督条件。左侧视频提供外观和时序变化；中间追踪记录增加人体几何信息；右侧机器人交互才能直接对应特定具身的动作与反馈。三栏不是自动转换流水线，人体姿态仍需对齐、重定向和验证。")
...
具体而言，EgoMimic 把人体手轨迹与机器人末端轨迹变换到观测时刻的相机参考系，共享位姿预测接口；机器人数据额外监督关节动作输出。人和机器人分别做动作分布归一化，视觉侧还处理手臂外观差异。因此，“共享策略”包含有意设计的共用部分与专用部分，并不是直接拼接两份形状相同的数组。[论文 Table I、Figure 4 与 Algorithm 1](https://arxiv.org/html/2410.24221v1)
...
### 7.6 把“用了人类视频”拆成可比较的训练接口 {#human-supervision-interfaces}

选方案时，先确定自己能提供哪一种监督，再比较模型名字。下表归纳前文方法，各行对应的目标并不完全相同：

| 方法 | 人类数据中的关键监督 | 与机器人连接的位置 | 实施前应确认什么 |
| --- | --- | --- | --- |
| MimicPlay | 标定多视角得到的三维手轨迹、视觉目标 | 人类数据学高层计划，机器人示范学底层控制 | 能否取得可靠的三维轨迹与目标条件 |
| EgoMimic | 第一视角、SLAM 与手部轨迹 | 共享位姿输出；机器人关节输出另有监督 | 相机参考系、分布归一化与专用输出头 |
| EgoHumanoid | 头部／身体追踪、第一视角及对齐后的动作 | 人机数据在对齐接口上共同训练 | 移动、手臂、夹爪和视角怎样同时对齐 |
| EgoScale | 大规模估计／重定向的人体动作 | 人类预训练，再经人机对齐与任务后训练迁移 | 数据标签质量、各阶段资源与目标手部接口 |
| LAPA | 从相邻图像变化学习的潜在动作 | 视频预训练后，用机器人动作数据落地 | 潜在编码学到什么，以及如何对接物理动作 |
{.table-readable}

例如只有单目生活录像时，可以先考虑表征或潜在动作路线；有可靠三维手轨迹后，才具备上述几何监督路线所要求的一部分条件。即使同一段原始录像能经过不同处理进入多行，也应分别记录处理版本、标签来源与可用比例。
...
**MoMaGen**将数据生成推进到全身移动操作：根据新场景调整末端目标，还要协调底盘放置、躯干与观察。[项目与论文入口](https://momagen.github.io/)
...
![从照片或视频建立场景后，必须补足碰撞、质量关节、控制传感器与任务条件，再执行和验证，才能产生带动作的训练轨迹](assets/scene-to-rollout.svg "图 6：从空间资产到训练轨迹的工程检查链。不是对某一产品内部实现的描述；真实效果最终需要独立的机器人闭环评测。")

<span id="fei-fei"></span>

## 9. 从采集到预测：不同研究方向怎样衔接 {#research-connections}

前面的方法可以按它们解决的数据问题串联起来。这里的连接表示工程上的分工，不表示这些项目共享同一套训练数据、接口或模型权重。

| 工作／方向 | 主要贡献所在 | 与下一阶段怎样连接 |
| --- | --- | --- |
| [XRoboToolkit](https://xr-robotics.github.io/)、[BRS／JoyLo](https://behavior-robot-suite.github.io/) | 把人的操作组织成机器人示范 | 保留控制链、时间与反馈，为动作监督提供执行依据 |
| [UMI](https://umi-gripper.github.io/)、[EgoMimic](https://egomimic.github.io/) | 缩小人体示范与机器人接口的差异 | 通过工具或表示对齐，使部分人体运动可用于机器人学习 |
| [MimicPlay](https://mimic-play.github.io/) | 从人体运动中学习高层计划 | 结合机器人示范学习执行，连接任务结构与底层动作 |
| [BEHAVIOR-1K](https://behavior.stanford.edu/)、[RoboCasa365](https://robocasa.ai/) | 提供任务、场景与物理交互环境 | 为采集、生成和策略评测定义可执行条件 |
| [MimicGen](https://mimicgen.github.io/)、[MoMaGen](https://momagen.github.io/) | 从源示范扩展到新配置 | 通过仿真执行检查新轨迹，同时保留派生关系 |
| [SimFoundry](https://research.nvidia.com/labs/gear/simfoundry/)、[World Labs](https://www.worldlabs.ai/about) 的空间建模方向 | 利用真实视觉构建或生成场景 | 是否能进入控制评测，还取决于交互属性、动作接口与验证方式 |
{.table-readable}

一种可能的实施顺序是：先用遥操作取得目标机器人的执行依据，再利用人类数据补充行为覆盖，用仿真扩展任务配置，最后以独立交互数据验证策略和世界模型。实际项目不必完整经过每一步；应该从当前缺少的监督出发选择模块。

这也解释了为什么不能只用“数据量更大”比较各条路线：计划学习增加的是任务与运动结构，仿真生成增加的是可执行配置，空间建模增加的是环境表示。只有当新增信息能通过明确接口进入目标模型，并在独立任务中产生收益时，这些工作才形成有效衔接。
...
![同一抓取初态分成两条动作序列：左侧移动到托盘后释放，右侧提前张开夹爪使物块落在托盘外](assets/world-model-action-outcomes.png "图 7：动作条件预测需要区分不同动作带来的后果。上方是共同初态；左下在托盘上方释放，右下提前释放。半透明姿态表示各分支中的中间动作。这是解释动作与后果关系的示例，不是某个世界模型的预测画面或实测成功率。")
...
离线处理还要区分**标签可以利用的信息**与**策略输入在运行时可获得的信息**。全序列 SLAM、双向平滑或看完整视频后生成的标注，可以帮助制作训练目标；若把这些结果当成当前时刻的在线输入，则需要匹配部署时的估计器及其延迟。否则，训练输入中的当前状态可能暗含未来帧提供的信息。
...
程序只使用标准库与人工构造的小例子，不连接机器人、不下载模型，也不是某种数据格式的完整验证器。来源检查只验证声明的元数据，不能证明硬件真正执行；真实系统还需核对反馈。[本次运行结果](lab-results.json)保存了 30 项检查与关键数值。
...
| 来源混合 | 各采样 100 个窗口，人类动作仅 25 个有效 | 统一有效元素均值与分别按来源取均值，得到不同目标 |
...
![图像 A 在决策前已到达，图像 B 尚未到达；下方的未来动作窗口作为训练目标而非提前可见的输入](assets/observation-time.svg "图 8：数据契约实验中的人工时间示例。横轴为校准后的公共时钟，数值不是 PICO 或任何机器人的实测延迟。")
...
### 11.8 RLDS 与 LeRobot：格式转换后还要检查什么 {#dataset-format-boundaries}
...
实际转换时，建议抽查一条成功轨迹、一条超时轨迹和一条设备中断轨迹：逐一比较首尾观测、有效动作数、结束原因、相机帧映射与转换前后的动作单位。记录数据格式版本和转换器提交；“脚本运行成功、张量形状正确”只证明可读取，还没有证明监督含义保持不变。

### 11.9 第一批试采数据怎样交付与验收 {#collection-delivery}

采集端与训练端第一次对接时，可以按下表交付。它是本文的工程建议，适用于前文几种路线，不是新增的数据格式标准。

| 交付物 | 最少说明什么 | 接收方怎样复核 |
| --- | --- | --- |
| 会话与设备记录 | 设备型号、软件版本、场景、操作者会话、开始／结束原因 | 能追溯到原始记录，并识别跨集合的同源数据 |
| 控制与标签字典 | 维度、单位、坐标系、绝对／增量、命令／反馈／估计来源 | 抽取短片段，逐字段对应控制链与画面 |
| 时间与标定材料 | 原始时钟域、换算方式、外参方向、重置事件和版本 | 复算时间映射与位姿变换，检查重置前后窗口 |
| 轨迹索引与有效性 | episode 范围、相机对应关系、动作掩码、终止与截断 | 重建首尾样本，确认不跨轨迹或把缺失当零 |
| 试采质量报告 | 原始量、各阶段剔除量、延迟分布、跟踪失效、任务结果 | 同时抽查保留与剔除片段，解释每项统计的分母 |
{.table-readable}

三种硬件路线的关键证据也不同。**PICO／VR**要确认追踪输入经过什么重定向、控制器实际接受什么；**主从**要核对主手目标与从手实测，不能只交主手关节；**UMI**要交代工具轨迹重建、夹爪开度与质量筛选，不能把手持工具运动写成已执行的机器人命令。普通人类视频可以明确交付“无机器人动作”，并说明重建或伪标签的生成过程。

验收阈值应由目标任务确定。例如要求同步残差小于某个数值之前，要说明如何测量残差、采用哪个时间参考、误差是否包含运动响应；需要接触监督时，也要指定传感器与有效性标准。**字段齐全、数值自洽和真机任务成功是三层证据**，每层都应有对应报告，不能由其中一层替另一层背书。
...
**采样比例、有效标签比例和损失权重是三个不同的量。** 若采用统一的有效动作元素池，且不同来源的动作维度与窗口长度相同，那么来源 $s$ 的加权监督份额可近似记为：

$$
w_s=\frac{p_s r_s\lambda_s}{\sum_j p_j r_j\lambda_j},
$$

其中 $p_s$ 为采样概率，$r_s$ 为采样后有效动作标签的比例。这描述的是该归约规则下的监督权重，不是梯度贡献或最终性能。

看一个人工例子：各采样 100 个机器人和人类窗口，机器人窗口全部有效，而人类窗口只有 25 个具有当前动作头可用的标签。等权合并后，人类数据在有效动作窗口中占 **25 / 125 = 20%**。若把人类动作损失权重设为 4，加权份额才变成 50%；这只是算术演示，并不说明权重 4 最合适。

如果实现改为“先对每个来源的有效标签分别取均值，再等权相加”，上述两个来源已经各占一半损失权重。还要区分按坐标、时间步、窗口还是来源取均值。本文实验包加入了对应的监督份额检查；训练日志也应记录采样数、有效标签数和实际归约方式。

实验令机器人有效元素的平方误差为 0、人类有效元素为 1：统一取均值为 **0.2**，按来源分别取均值后等权平均为 **0.5**。人类权重为 4 时，除以有效权重总和得到 **0.5**；若仍除以未加权的 125 个有效元素，则得到 **0.8**。后两者的来源相对份额相同，但整体损失尺度不同，会影响与其他损失项的平衡。
...
### 12.7 把人类数据的收益拆成可回答的实验 {#data-ablation-plan}

假设已经有一份固定的机器人训练集，下面是一组可逐步执行的实验设计。它是本文建议，不是某篇论文的原始实验表：

| 实验 | 数据与训练设置 | 主要回答的问题 |
| --- | --- | --- |
| R0：机器人基线 | 固定机器人数据与基础训练预算 | 当前策略的能力和主要失败在哪里 |
| R1：增加训练预算 | 重用同一机器人数据，增加到混合方案所需预算 | 改善是否仅来自更多优化或计算 |
| H0：同场景人类数据 | 在相应预算下加入机器人训练场景内的人类示范 | 人体行为与动作监督本身是否有帮助 |
| H1：多场景人类数据 | 与 H0 保持人类有效数据量接近，增加场景覆盖 | 收益是否来自新增场景与视觉分布 |
{.table-readable}

每组都应同时在“机器人见过的场景”“只有人类数据覆盖的场景”和“所有来源都未见的场景”报告结果。对第二类，要排查机器人数据及其派生样本是否已经覆盖；对第三类，则要排查所有训练来源。对于无法完整审计的预训练语料，应把已核对范围写清楚。

**等计算预算与等机器人数据曝光量是不同的实验约束。** 固定总步数和 batch 大小时，插入人类样本可能减少机器人样本被训练的次数；保持机器人曝光次数，又可能增加总计算。应记录实际见过的机器人窗口数、人类有效窗口数、训练步数及计算成本，分别建立对照，而不是把所有变化都归因于“数据更好”。
...
[下载论文与项目阅读索引](source-map.json)，可按类别检索 37 项论文／项目，以及固定源码版本、设备文档与两项数据格式文档；[图示生成脚本](make_figures.py)可以重建本文的三张 SVG。索引只列阅读入口与证据边界，不代表各论文完整训练数据都已公开。
...
- 用同一批人机样本解释有效元素、来源均值与权重归一化的区别，并设计等预算对照。
