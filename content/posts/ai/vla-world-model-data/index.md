...
**等计算预算与等机器人数据曝光量是不同的实验约束。** 固定总步数和 batch 大小时，插入人类样本可能减少机器人样本被训练的次数；保持机器人曝光次数，又可能增加总计算。应记录实际见过的机器人窗口数、人类有效窗口数、训练步数及计算成本，分别建立对照，而不是把所有变化都归因于“数据更好”。 若改变总训练步数，还要检查学习率是否随之改变；[X-VLA 的训练预算与余弦调度对照]({{< relref "/posts/ai/x-vla" >}}#training-budget)给出了数值例子和常见框架的配置区别。
