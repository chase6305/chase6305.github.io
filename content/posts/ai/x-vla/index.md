...
![相同观测对应正负两个标签，隐藏域条件时单一回归输出取平均，加入域条件后共享模型可区分两种目标](assets/domain-conditioning.png "图 1：等权标量 MSE 算例。右侧省略了不变的观测输入，A/B 表示两次独立样本的域条件；它们共同使用一个策略，不表示同时输入两个域。")
...
![X-VLA 的主相机与语言、辅助视角、动作状态时间及域软提示进入共享动作 Transformer](assets/soft-prompt-architecture.png "图 2：Domain A/B/C 是示意标签；主、辅视角共用视觉骨干，图中为表达两条特征路径而分开绘制。动作编码／解码器的域相关参数见正文。")
...
![训练将专家动作与噪声混合后预测干净动作；推理复用固定噪声逐步更新动作块](assets/action-refinement.png "图 3：干净动作预测与迭代重混合。曲线和点仅用于表达动作块，不是训练曲线、真实轨迹或成功率。")
...
![固定噪声重混合复用最初噪声，反向 Euler 更新当前路径状态，两者都利用最新的干净动作估计形成下一步输入](assets/remixing-vs-euler.png "图 4：两种迭代方法保留的状态。混合是按公式逐元素计算，点云仅表示信息来源；图中省略时间系数，也不比较两种方法的生成质量。")
...
![20 维动作由两个位置、旋转和夹爪槽位组成，经适配层进入机器人控制器并返回观测](assets/ee6d-control-loop.png "图 5：左下展示可采集的状态来源；标准 ee6d 的夹爪输入通道会被屏蔽。真实客户端的状态刷新方式需单独检查。")
