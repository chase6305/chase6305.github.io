...
| 理解闭环与数据接口 | [示范如何成为条件](#context)、[任务歧义](#task-ambiguity) |
...
### 3.3 一次示范可能对应多个任务解释 {#task-ambiguity}

继续使用自拟的放置任务：示范中，圆柱被放入左边的蓝盒。仅凭这一次成功，至少有三种规则都能解释观察：

| 候选任务规则 | 原示范是否支持 | 把蓝盒移到右侧后会怎样 |
| --- | --- | --- |
| 放入左边的盒子 | 支持 | 继续选择左侧容器 |
| 放入蓝色盒子 | 支持 | 改为选择右侧蓝盒 |
| 重复原来的绝对空间目标 | 支持 | 仍伸向旧位置，甚至可能错过容器 |

![同一段示范兼容位置规则和颜色规则；交换容器位置后，两种规则给出不同目标](assets/demonstration-ambiguity.png "使用 imagegen 生成的概念示意：下方两个场景是竞争假设，不是 GEN-1.5 实测结果。")
...
可以下载 [完整练习包](gen-1-5-lab.zip)，解压后进入 `gen-1-5-lab` 目录，按 [README](README.txt) 运行。也可以单独下载 [prompt_eval.py](prompt_eval.py) 和 [test_prompt_eval.py](test_prompt_eval.py)，放在同一目录。仅使用 Python 标准库：
