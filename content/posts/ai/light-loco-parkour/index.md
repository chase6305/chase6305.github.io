...
![同一个示意人形机器人依次接近平台、用双手与膝盖支撑攀爬、在平台上站稳，下方展示深度和本体观测经过一个循环策略输出关节目标](assets/whole-body-contacts-clean.png "图 1：机器人外形、平台与动作姿态均为示意，不是 Lightbot 0 的照片、论文轨迹或动力学验证结果。")
...
![一次脚部加速度超限后，累积量在多个控制周期内指数衰减；标注源码默认正号与论文负号的奖励差异](assets/reward-memory.svg "图 3：根据正文递推式计算的数值。纵轴是非负的超限累积量，不是已经乘权重的总奖励，也不是实机加速度测量。")
...
### 3.5 换控制频率会改变累积量的幅值 {#reward-time-step}

$\tau$ 决定历史在物理时间上衰减多快，但不单独决定这个累积量的幅值。若保持 $\tau=0.06$ s、每步超限量 $e=20$，只改变递推式中的控制周期，稳态分别为：

| 控制频率 | 每步 $\alpha$ | 稳态 $e/(1-\alpha)$ |
| --- | ---: | ---: |
| 50 Hz | 约 0.7165 | 约 70.5545 |
| 100 Hz | 约 0.8465 | 约 130.2776 |

两种设置下，一次相同初始冲击经过 60 ms 都衰减到原来的 $\exp(-1)$；持续超限的稳态幅值却相差约 1.8465 倍。原因是更密集地加入同样大小的 $e$，而递推式没有乘上步长或 $(1-\alpha)$。这些是 [parkour_lab.py](parkour_lab.py) 验证的数学结果，不是改变真实机器人频率后的实测。

因此，改变控制周期时，仅重新计算 $\alpha$ 不能保证奖励尺度不变；甚至只改频率、仍把 `hparams.dt` 留在 0.02，会连物理衰减时间也改变。若设计新奖励，可明确选择累积量、时间积分或保持常量输入幅值的平滑量，并相应调整权重；若复现固定实现，则应保留其定义，记录控制周期。两种目标不宜通过静默更换公式混在一起。
...
### 11.5 并行环境需要按行重置，不能“一起清零” {#selective-reset}

上面的生命周期问题在向量化环境里更具体。假设两个环境的脚部加速度记忆分别为 $[20,40]$，下一步都没有新增超限量，且只有环境 0 开始新 episode。沿用第 3.3 节的 $\alpha=\exp(-1/3)$，期望的新记忆是：

$$
[0,40\alpha]\approx[0,28.6613].
$$

不重置会得到约 $[14.3306,28.6613]$，让新 episode 继承旧冲击；调用当前 `reset_()` 则得到 $[0,0]$，同时丢掉仍在运行的环境 1 的历史。固定源码的 `FootAccelerationPenalty.reset_()` 将整份缓存设为 `None`，wrapper 再对全部有状态奖励调用它，没有 `done_mask` 参数。[奖励缓存与全局重置](https://github.com/lucidrains/light-loco-parkour/blob/963a6ec3b8b42eb29dd6b9dfed34ededd3c64c7b/light_loco_parkour/light_loco_parkour.py#L1194)

随文 [source_probe.py](source_probe.py) 已在两个环境的真实组件上验证这三组数值；其中按行重置的值是期望行为对照，**不是声称上游已经实现该接口**。扩展训练器时，可以为有状态组件设计逐环境重置接口，或按环境隔离状态；还要明确 batch 维的位置及设备、形状变化时的处理方式。

重置顺序同样重要：先用终止前的状态完成最后一步奖励和转移记录，再清理已结束环境的历史，然后处理自动重置后的首个观测。GAE 对截断所需的终点价值应来自该 episode 的终点条件，不能在隐藏状态清空后，用下一轮初始观测冒充它。

需要共同遵守这个边界的状态包括 GRU 隐藏状态、原始堆帧缓存、上一动作以及奖励记忆。训练片段若跨 reset，还需阻断循环状态传播和潜变量转移配对。可变 batch 若重排环境，也应同步重排这些状态；只保持张量形状相同，不代表每一行仍属于同一个机器人。
...
把本文与 [X-VLA]({{< relref "/posts/ai/x-vla" >}}) 和 [RTC]({{< relref "/posts/ai/real-time-chunking" >}}) 放在一起阅读时，可以沿三层接口比较：

| 问题 | 对应文章的主要切入点 | 不能从这一层自动得到什么 |
| --- | --- | --- |
...
- 用源码探针解释堆帧缓存、GRU 隐藏状态、逐环境重置及潜变量默认开关为什么影响结果。
