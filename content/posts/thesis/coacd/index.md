...
$$
H_b = \max\left(\sup_{p \in \partial S} d(p, \partial \operatorname{CH}(S)),
\sup_{q \in \partial \operatorname{CH}(S)} d(q, \partial S)\right)
$$

  其中，$\partial S$ 为网格表面，$\partial \operatorname{CH}(S)$ 为凸包表面。通过点-面距离优化采样效率，确保表面细节（如凹槽深度）被精确捕捉。
