...
脚本只依赖 Python 标准库，要求 CSV 至少包含 `time_s`、`force_n` 和 `position_m` 三列。它会输出基线位置、稳态位移、稳态外力、估计刚度、峰值位移、超调率、卸载后位置 RMS 和平均采样周期。例如默认参数的等效刚度应接近设定的 $1000\,\mathrm{N/m}$，但不能要求浮点结果恰好等于理论值。
