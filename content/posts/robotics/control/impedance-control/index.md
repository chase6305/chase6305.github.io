...
categories: ["机器人技术"]
authors: ["chase"]
summary: "从质量—弹簧—阻尼关系推导机器人阻抗控制，连接操作空间动力学、Pinocchio、整定和分阶段安全验证。"
showToc: true
TocOpen: true
comments: false
description: "从质量—弹簧—阻尼关系推导机器人阻抗控制，连接操作空间动力学、Pinocchio、整定和分阶段安全验证。"
contentLanguage: "zh-CN"
reading_prerequisites: "机器人动力学、Jacobian 与力矩接口"
reading_focus: "先明确力与运动的坐标系及符号，再完成仿真、限幅和回退检查。"
related_posts:
  - "/posts/robotics/dynamics/parameter-identification"
  - "/posts/pid"
---
...
这取决于任务真正要求什么。纯阻抗控制给定的是平衡位置和刚度，它只会间接产生接触力。以任务坐标系 $T$ 的法向 $z$ 为例，忽略摩擦和动态项后：

$$
f_z\approx k_z(z_d-z)
$$

如果工件表面位置 $z_s$ 已知且足够准确，可以由期望接触力反推平衡点：

$$
z_d\approx z_s+\frac{f_{z,d}}{k_z}
$$

但只要表面位置、环境刚度或工具长度发生变化，实际力就会偏离 $f_{z,d}$。因此“设置一个位置偏移”不是严格的力控制，也不能用力矩限幅代替接触力上限。需要稳定保持指定法向力时，应测量或可靠估计 $f_z$，在法向使用力反馈；其余方向仍可使用阻抗：

$$
S=\operatorname{diag}(1,1,0,1,1,1),\qquad
F=S F_{imp}+(I-S)F_{force}
$$

这里假设六维顺序是 `[x,y,z,rx,ry,rz]`，因此第三维由力环负责。一个简单法向力环可写成：

$$
f_{force,z}=f_{ff,z}+k_{pf}(f_{z,d}-\hat f_z)
+k_{if}\int(f_{z,d}-\hat f_z)\,dt
$$

该式只展示结构。符号取决于本文前面冻结的“环境作用于机器人”约定；真机前必须用很小的正向命令验证。力环增益也不能照搬位置阻抗增益，两者单位和闭环对象都不同。
...
| --- | --- | --- | --- |
| 自由空间接近 | 低速位置/阻抗目标，不积分力误差 | 保持任务轨迹 | 力超过 $F_{on}$ 且速度方向合理 |
| 接触确认 | 锁存接触点，停止继续推进旧位置目标 | 锁存当前切向参考 | 条件持续 $T_{on}$，信号无饱和 |
| 建立接触力 | 将 $f_{z,d}$ 从当前测量值斜坡到目标值 | 切向低速或静止 | 力误差进入允许带 |
| 稳态作业 | 法向力反馈，并限制积分和功率 | 切向阻抗执行打磨/插入 | 作业完成、失去接触或超限 |
| 退出接触 | 先将力目标斜坡降到零，再沿法向撤离 | 保持切向与姿态连续 | 力低于 $F_{off}$ 持续 $T_{off}$ |

从阻抗切到力反馈时，应让新力环的初始输出等于切换前的法向 wrench，或者先令 $f_{z,d}=\hat f_z$ 再缓慢改变目标。否则即使两个控制器各自稳定，切换瞬间也会产生阶跃。退出时先卸载再撤离，能够避免积分残留把机器人重新压向工件。

期望作业力、允许峰值力和触发急停的力必须是三个独立参数。例如 $20\,\mathrm N$ 的作业目标不意味着 $20\,\mathrm N$ 就应触发故障；同时也不能只依靠 $\tau_{max}$ 推断末端安全力，因为 $J(q)$ 会使同一关节力矩限制在不同构型对应不同 wrench 能力。
...
第二个关系比直接检查 $JN^T$ 更重要，因为 $N^T$ 投影的是关节力矩，而力矩通过 $M^{-1}$ 才形成关节加速度。可以定义一个无量纲投影残差：

$$
r_N=
\frac{\left\|JM^{-1}N^T\right\|_F}
{\left\|JM^{-1}\right\|_F\left\|N^T\right\|_2+\epsilon}
$$

还可以随机生成投影前力矩 $\tau_0$，令 $\tau_{null}=N^T\tau_0$，直接比较投影前后的任务加速度泄漏：

$$
r_{leak}=
\frac{\left\|JM^{-1}\tau_{null}\right\|_2}
{\left\|JM^{-1}\tau_0\right\|_2+\epsilon}
$$

若随机样本的分母接近零，说明 $\tau_0$ 本身几乎不激励任务方向，应丢弃该样本而不是把 $r_{leak}$ 当作有效结果。测试应覆盖大量安全构型、不同任务维度以及接近奇异但仍允许运行的构型，而不是只在一个中间姿态测试。$M$、$J$、$N$ 必须来自同一个 $q$、同一模型版本和同一任务 frame。

使用阻尼伪逆时，$JJ^{\#}$ 不再严格等于 $I$，零空间也会产生可预期的任务泄漏；阻尼越大，数值更平稳，但任务与零空间隔离越弱。因此验收阈值应随奇异值或阻尼调度定义，不能固定要求机器精度。反过来，如果远离奇异位形仍有明显泄漏，优先检查投影转置、质量矩阵、Jacobian frame 和矩阵求解实现。

即使投影前满足上述关系，随后逐关节裁剪总力矩也会改变 $\tau_{null}$ 的方向，使零空间重新影响主任务。可以在限幅后计算实际任务力矩残差，或使用带力矩约束的分层 QP，把主任务优先级、零空间目标和关节边界同时放入求解器。零空间验证必须覆盖“投影输出”和“最终发送力矩”两个位置。
...
如果任务只需要沿一个已定义方向施力，可以在逐关节裁剪之前计算该方向的可行区间。设 $a\in\mathbb R^6$ 表示任务 frame 中一个纯力方向，标量幅值为 $f$，则：
...
其中 $\tau_{fixed}$ 是本周期已经被重力补偿、零空间、摩擦前馈等占用的力矩。即使重力补偿由驱动器内部完成，只要这里约束的是电机或减速器的物理总力矩，预算中仍应扣除驱动器预计会添加的部分；只有约束明确针对“上层附加力矩命令”时，才按接口定义排除它。令：

$$c=J^Ta$$

对每个关节的非对称力矩限制 $\tau_{min,i}\le\tau_i\le\tau_{max,i}$，都可得到一个标量区间：

$$
\frac{\tau_{min,i}-\tau_{fixed,i}}{c_i}
\le f\le
\frac{\tau_{max,i}-\tau_{fixed,i}}{c_i},\qquad c_i>0
$$

当 $c_i<0$ 时交换上下界；当 $c_i\approx0$ 时，该关节不限制这个方向的 $f$，但仍需确认 $\tau_{fixed,i}$ 自身没有越界。所有关节区间取交集后得到 $[f_{min},f_{max}]$。若交集为空，说明仅固定项就已冲突，或者当前构型无法在约束内产生该方向的 wrench，控制器应降级而不是继续裁剪。

例如某个纯力方向对两个关键关节的力臂系数为
$c=[0.2,0.5]\,\mathrm m$，扣除重力与零空间后正向剩余力矩分别为
$[10,15]\,\mathrm{N\cdot m}$，则正向力上界为：

$$
f_{max}=\min\left(\frac{10}{0.2},\frac{15}{0.5}\right)
=30\,\mathrm N
$$

第二个关节先成为瓶颈。实际命令还应乘小于 1 的安全系数，并为模型误差、摩擦、碰撞瞬态和底层控制器保留余量。

同样的方法也可纳入力矩变化率限制：先将本周期允许区间收紧为

$$
[\tau_{lower,k},\tau_{upper,k}]
=[\tau_{min},\tau_{max}]
\cap[\tau_{cmd,k-1}-\dot\tau_{max}\Delta t,
      \tau_{cmd,k-1}+\dot\tau_{max}\Delta t]
$$

再计算 $f$ 的区间。这样得到的是“本周期能够实现多少”，可能显著小于静态力矩上限。$J(q)$ 和 $\tau_{fixed}$ 都随状态变化，因此可行区间需要在线更新；配置中的 `wrench_abs_limit` 只应作为额外的任务级上限。

这里的 $a$ 应代表物理意义明确的纯力轴或纯力矩轴。不要把 N 与 N·m 混在同一个欧氏单位向量里归一化。一般六维 wrench、多方向优先级和零空间同时存在时，应改用带约束 QP；上述一维区间法适合快速检查单方向接触力是否可能实现。
...
阻抗刚度主要决定接触建立后的力—位移关系，而接触瞬间还携带运动动能。沿接触方向 $a$ 的等效质量可近似写为：

$$
m_{eff}=\left(a^TJM^{-1}J^Ta\right)^{-1}
$$

若机器人方向刚度为 $K_d$、环境刚度为 $K_e$，把两者暂时看作串联弹簧，则等效刚度为：

$$
K_{eq}=\left(K_d^{-1}+K_e^{-1}\right)^{-1}
$$

忽略阻尼、控制延迟、结构高频模态和主动卸载，仅用能量守恒估算，接近速度 $v_0$ 对应的碰撞峰值约为：

$$
\frac{1}{2}m_{eff}v_0^2\approx\frac{F_{peak}^2}{2K_{eq}},
\qquad
F_{peak}\approx v_0\sqrt{m_{eff}K_{eq}}
$$

该式不是安全认证模型，但能说明三个直接结论：速度翻倍会近似使峰值力翻倍；高等效质量构型更难安全停止；环境越硬，降低机器人刚度越重要，但不能替代限速。若试验给定允许碰撞力 $F_{allow}$，一个保守候选速度起点为：

$$
v_{candidate}\le
\frac{F_{allow}}{\sqrt{m_{eff,max}K_{eq,max}}}
$$

$m_{eff,max}$ 和 $K_{eq,max}$ 应取任务工作空间及允许工件集合中的不利值，并再乘安全系数。方向 $a$ 在这里应是纯平移单位方向；旋转碰撞需要使用一致的转动惯量和力矩单位重新推导。

检测和执行延迟还会让机器人在发现接触后继续前进。端到端延迟为 $T_d$ 时，额外侵入量的第一阶估计为：

$$
\Delta x_{delay}\approx v_0T_d
$$

在刚性环境中，即使这个距离很小，也可能造成明显附加力。实际 $T_d$ 必须包含传感器采样、滤波、通信、状态机确认时间、控制计算、命令下发和执行器响应，不能只使用控制循环周期。前文的 $T_{on}$ 滞回能够抑制误触发，但也会增加确认延迟，因此接触检测阈值、持续时间和接近速度必须联合设计。

最终安全边界应通过受控碰撞试验确认，并同时记录峰值力、冲量、停止距离、关节力矩、饱和率和接触持续时间。只展示低刚度数值或稳态 $F=K\delta x$，不足以证明机器人在运动接触中安全。
...
静态加载只能验证刚度，不能判断阻尼和表观惯量是否正确。对单一方向施加小幅正弦外力，并记录稳态位移响应：

$$
F(t)=F_0\sin(\omega t),\qquad
\delta x(t)=X_0\sin(\omega t-\phi)
$$

对理想二阶阻抗
$M_d\delta\ddot x+D_d\delta\dot x+K_d\delta x=F$，定义从位移到外力的动态刚度：

$$
Z_x(j\omega)=\frac{F(j\omega)}{\delta X(j\omega)}
=K_d-M_d\omega^2+jD_d\omega
$$
...
| 频响特征 | 主要对应参数 | 诊断含义 |
| --- | --- | --- |
| $\omega\to0$ 时的 $\operatorname{Re}(Z_x)$ | $K_d$ | 低频力—位移斜率 |
| $\operatorname{Im}(Z_x)/\omega$ | $D_d$ | 阻尼及速度滤波影响 |
| $\operatorname{Re}(Z_x)$ 对 $\omega^2$ 的负斜率 | $M_d$ | 实际表观惯量是否符合设计 |
| 幅值突峰或相位快速下降 | 未建模模态、延迟或接触共振 | 需要降低工作带宽或修改结构 |

每个频点应先等待过渡过程衰减，再截取整数个周期，通过同频正弦/余弦投影或 FFT 得到复数 $F(j\omega)$ 与 $\delta X(j\omega)$。还应计算相干性或至少检查该频率分量是否明显高于噪声；当位移幅值接近编码器分辨率时，直接相除会产生没有意义的巨大阻抗。

建议采用逐点正弦或幅值受限的 multisine，从低频开始逐步提高，并始终满足速度、加速度、接触力、关节力矩和变化率限制。扫频过程中一旦出现饱和，系统已变为非线性，不能再用该频点拟合理想 $M_d,D_d,K_d$。同样要避免把环境自身的刚度和阻尼误认为控制器阻抗；可先在仿真或已知柔顺度的测试装置上验证，再测试目标工件。

实际结果不应只报告一组三个拟合数字。至少保存 $|Z_x|$、相位、实部、虚部、相干性、输入幅值和每个频点的饱和状态，并与期望曲线及允许误差带叠加。若低频刚度正确而高频偏差明显，优先检查控制延迟、速度滤波、执行器带宽和结构柔性，而不是重新调整静态刚度。

可以下载并运行[频响辨识示例 `impedance_frequency_response.py`](impedance_frequency_response.py)：

```bash
python3 impedance_frequency_response.py
```

脚本只依赖 Python 标准库。它分别仿真 $0.5$～$5\,\mathrm{Hz}$ 的正弦外力，丢弃前 8 个周期，再对后 8 个周期进行单频 DFT，最后根据
$\operatorname{Re}(Z_x)=K_d-M_d\omega^2$ 和
$\operatorname{Im}(Z_x)=D_d\omega$ 回归参数。脚本末尾会检查估计值与仿真真值的相对误差是否小于 $2\%$。这只是理想线性基线；真机版本还需要窗口函数、相干性、异常区间排除、置信区间和独立重复试验。
...
为了把公式落实到可运行示例，可以先运行第 7.1 节的一维仿真，再用[日志分析脚本 `analyze_impedance_log.py`](analyze_impedance_log.py)计算指标：

```bash
python3 impedance_1d.py
python3 analyze_impedance_log.py impedance_response.csv \
  --step-on 0.5 --step-off 1.5 \
...
输入检查会拒绝重复表头、缺失数值、NaN/Inf 和非递增时间戳；直接调用 `analyze()` 也遵守相同约束。验收时非有限指标必须判为失败，不能只比较 `actual > limit`：NaN 与阈值的比较会返回假，可能使无效结果被误报为通过。可先把 CSV 的一个位置值改成 `nan`，确认脚本以非零状态退出，再使用正常数据复测。
...
脚本默认使用施力阶段最后 $20\%$ 的样本估计稳态值。这要求施力时间足够长；如果系统在该窗口仍未稳定，计算出的“等效刚度”会混入动态响应。此最小示例假定基线外力已归零；有预载时应先定义增量力与增量位移。平均采样周期也不能代替逐间隔抖动或丢帧检查。真机数据还应先排除状态切换、碰撞尖峰和饱和区间，并将排除规则写入报告。分析工具负责计算指标，是否通过由试验前冻结的阈值决定；本脚本读取 JSON，下文参数包中的 `acceptance.yaml` 是工程组织示意，不能直接传给该脚本。
...
零偏不能在工具任意姿态下简单把六维读数清零，因为此时读数中包含工具重力。常用做法是在无接触状态采集多个差异明显的姿态，联合估计传感器零偏与工具质量、质心；如果工具参数已经可信，也可以先计算每个姿态的工具重力 wrench，再对残差求零偏。动态补偿还需要同步的 $q,\dot q,\ddot q$ 与工具惯量，时间错位会把正常加速误判成碰撞。

滤波位置也有含义：先补偿再滤波，便于保留同一物理量的带宽；但模型量和传感器量必须采用相容延迟。截止频率不能只按“曲线更平滑”选择，应同时测量噪声标准差、接触阶跃上升时间和闭环相位滞后。安全碰撞通道可使用独立、更高带宽且更保守的阈值，不能完全依赖用于精细力控的慢滤波信号。

正式接触实验前，可做以下无接触验收：

| 测试 | 应观察的结果 | 失败时优先检查 |
| --- | --- | --- |
| 多姿态静止 | 补偿后的均值接近零，且不随工具朝向系统变化 | 质量、质心、外参和重力方向 |
| 同姿态重复上电 | 零偏落在批准范围内 | 预热、温漂、安装预紧和线缆受力 |
| 低速往返运动 | 残差不随速度方向出现大幅翻转 | 时间对齐、线缆拖曳和摩擦串扰 |
| 保守加减速 | 惯性补偿后峰值明显降低且没有反向放大 | 惯量、$\ddot q$ 质量和符号 |
| 已知方向轻推 | 估计力的轴与方向正确，卸载后回零 | frame、六维排列和力臂变换 |

验收报告至少保存每轴均值、标准差、峰值、温度、姿态范围、工具版本和滤波参数。接触阈值应基于这些数据设置，例如高于无接触噪声与模型残差的统计上界并保留工程裕量，而不是为所有工具固定同一个经验数字。
...
- 统一位置、速度、力矩和 wrench 的坐标系与单位，先在仿真中观察阻抗参数对响应的影响。
- 比较有无重力补偿、接触切换和力矩饱和时的行为；静态刚度曲线合理不等于接触过程满足稳定性与安全要求。
